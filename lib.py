...
class model():
    ...
    def plot_density(self, dynamic=True, log_plot=False, nb_images=6):
        Z = self.Z
        Us = self.Us
        dt = self.dt
        T  = self.T
        ...
        if THETA is not None and dynamic:
            curve_dict = {round(dt * i, ndigits=2): hv.Curve((Z, Us[i])) * hv.VLine(THETA[i]).opts(title = "Phenotypic density",xlabel = "Phenotype",color='orange') for i in range_plot}
        else:
            curve_dict = {round(dt * i, ndigits=2): hv.Curve((Z, Us[i])).opts(title =f"Phenotypic density", xlabel = "Phenotype") for i in range_plot}
    ...
    #         curve_dict = {i: hv.Curve((Z, Us[i])).opts(title=f'Approximated solution at t = {dt * i:.1f}') for i in range(0, Us.shape[0], time_step)}
        ...
        options = {'title': 'Phenotypic density', 
                   'xlabel': 'Phenotype', 'ylabel': 'Time', 
                   'colorbar': True, 'toolbar': None, 'width': 400}
        
        plot2 = hv.Image((Z, time, self.Us))
